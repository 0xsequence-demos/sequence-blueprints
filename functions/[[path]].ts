...
function serveCachedResponse(response: Response, age: number, now: number, isFresh: boolean) {
  ...
  // delete the cdn-cache-control header as this was previously set before saving to the worker cache
  // as this is needed to tell the worker cache how long to keep the response for, but when we
  // actually serve it, its very important we remove this header as it will otherwise cause
  // conflicts with the default cloudflare caching system.
